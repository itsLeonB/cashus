...
const NewGroupExpense: React.FC = () => {
  ...
  const [items, setItems] = useState<NewExpenseItemRequest[]>([
    createEmptyExpenseItem(),
  ]);
  ...
  const getSelectedPayerName = () => {
    ...
    const selectedFriend = friends.find(
      (friend) => friend.profileId === selectedPayerId
    );
  ...
  const updateItem = (
    index: number,
    field: keyof NewExpenseItemRequest,
    value: string | number
  ) => {
  ...
  const updateOtherFee = (
    index: number,
    field: keyof NewOtherFeeRequest,
    value: string
  ) => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    const validationError = validateGroupExpense(
    ...
    if (
      selectedPayerId !== "me" &&
      selectedPayerId !== "" &&
      !friends.find((f) => f.profileId === selectedPayerId)
    ) {
      setError(
        'Selected payer is not valid. Please select a valid friend or "Me".'
      );
      return;
    }
    ...
    try {
      ...
      navigate("/group-expenses", {
        state: {
          message:
            "Draft group expense created successfully. You can now assign participants to each item.",
        },
      });
    } catch (err) {
      setError(handleApiError(err));
      toast.error(`Error creating group expense: ${errToString(err)}`);
    } finally {
      setLoading(false);
    }
  };
                ...
                <div className="flex items-start mt-2">
                  ...
                  <div className="ml-2">
                    <p className="text-sm text-gray-500">
                      Select who actually paid for this expense. This helps
                      track who owes money to whom.
                      {friends.length === 0 && (
                        <span className="block text-amber-600 mt-1">
                          💡 Add friends to select them as payers for shared
                          expenses.
                        </span>
                      )}
                    </p>
                      ...
                      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                        <div>
                          ...
                          <input
                            ...
                            value={fee.name}
                            onChange={(e) =>
                              updateOtherFee(index, "name", e.target.value)
                            }
                            className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500"
                        ...
                        <div>
                          ...
                          <input
                            ...
                            value={fee.amount}
                            onChange={(e) =>
                              updateOtherFee(index, "amount", e.target.value)
                            }
                            className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500"
                            placeholder="0.00"
                            required
                        ...
                        <div>
                          ...
                          {fee.calculationMethod &&
                            feeCalculationMethods.find(
                              (m) => m.name === fee.calculationMethod
                            )?.description && (
