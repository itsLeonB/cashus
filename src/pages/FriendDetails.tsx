    ...
    return (
      <div className="min-h-screen bg-gray-50 flex items-center justify-center">
        <div className="text-center">
          <div className="text-red-600 text-lg mb-4">{error || 'Friend not found'}</div>
          <button
            onClick={handleBack}
          ...
          <div className="flex items-center justify-between py-6">
            <div className="flex items-center">
              ...
              <div>
                ...
                <p className="text-gray-600 capitalize">{friend.type.toLowerCase()} friend</p>
              </div>
            </div>
            <div className="flex space-x-3">
              <button
                onClick={() => navigate(`/transactions/new?friendId=${friend.profileId}`)}
        ...
        <div className="px-4 py-6 sm:px-0">
          ...
          <div className="bg-white overflow-hidden shadow rounded-lg mb-6">
            <div className="px-4 py-5 sm:p-6">
              <div className="text-center">
                <div className={`text-4xl font-bold mb-2 ${getBalanceColor(balance.netBalance)}`}>
            ...
            <div className="p-6">
              ...
              {activeTab === 'overview' && (
                <div className="space-y-6">
                  {/* Friend Info */}
                  <div>
                    <h3 className="text-lg font-medium text-gray-900 mb-4">Friend Information</h3>
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                      <div>
                        <label className="block text-sm font-medium text-gray-500">Name</label>
                        <p className="mt-1 text-sm text-gray-900">{friend.name}</p>
                      </div>
                      <div>
                        <label className="block text-sm font-medium text-gray-500">Type</label>
                        <p className="mt-1 text-sm text-gray-900 capitalize">{friend.type.toLowerCase()}</p>
                      </div>
                      {friend.email && (
                        <div>
                          <label className="block text-sm font-medium text-gray-500">Email</label>
                          <p className="mt-1 text-sm text-gray-900">{friend.email}</p>
                        </div>
                      )}
                      {friend.phone && (
                        <div>
                          <label className="block text-sm font-medium text-gray-500">Phone</label>
                          <p className="mt-1 text-sm text-gray-900">{friend.phone}</p>
                        </div>
                      )}
                      <div>
                        <label className="block text-sm font-medium text-gray-500">Friends since</label>
                          ...
                          <div className="flex items-center">
                            ...
                            <div>
                              ...
                              <p className="text-xs text-gray-400">
                                {transaction.transferMethod} • {format(new Date(transaction.createdAt), 'MMM dd, yyyy HH:mm')}
                              </p>
                            </div>
                          </div>
                          <div className="text-right">
                ...
                <div className="space-y-6">
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
                    ...
                    <div className="bg-orange-50 p-4 rounded-lg">
                      ...
                      <div className="text-sm text-orange-800">Days as Friends</div>
                    </div>
                  </div>
                  ...
                  {stats.firstTransactionDate && (
                    <div>
                      ...
                      <div className="space-y-2 text-sm text-gray-600">
                        <p>First transaction: {format(new Date(stats.firstTransactionDate), 'MMMM dd, yyyy')}</p>
                        {stats.lastTransactionDate && (
                          <p>Last transaction: {format(new Date(stats.lastTransactionDate), 'MMMM dd, yyyy')}</p>
                        )}
                      </div>
                    </div>
                  )}
